...
interface ProductGridProps { searchQuery: string; }
...
export default function ProductGrid({ searchQuery }: ProductGridProps) {
  ...
  return (
    <section className="py-10 bg-gradient-to-b from-cyan-500 via-teal-400 to-blue-600">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-2xl font-extrabold text-white">Săn Deal</h2>
          <div className="flex items-center gap-2 text-white/90">
            {["52", "46", "41"].map((n, i) => (
              <span key={i} className="bg-white text-cyan-600 font-bold rounded-md px-2 py-1 leading-none">{n}</span>
            ))}
          </div>
        </div>
