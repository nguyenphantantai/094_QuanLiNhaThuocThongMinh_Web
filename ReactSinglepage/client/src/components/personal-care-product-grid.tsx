...
export default function PersonalCareProductGrid({ products }: PersonalCareProductGridProps) {
  ...
  const discountMoney = (p: PersonalCareProduct) => (p.originalPrice ? Math.max(0, parseInt(p.originalPrice) - parseInt(p.price)) : 0);
            ...
            <div key={p.id} className="bg-white rounded-xl overflow-hidden shadow-md border">
              ...
              <div className="px-4 pb-4">
                ...
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-xs bg-blue-600 text-white px-3 py-1 rounded-full">Chăm sóc cá nhân</span>
                </div>
