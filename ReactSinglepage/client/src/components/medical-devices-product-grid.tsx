...
export default function MedicalDevicesProductGrid({ products }: MedicalDevicesProductGridProps) {
  ...
  const discountMoney = (p: MedicalDeviceProduct) => (p.originalPrice ? Math.max(0, parseInt(p.originalPrice) - parseInt(p.price)) : 0);
          ...
          {products.map((p) => (
            <div key={p.id} className="bg-white rounded-xl overflow-hidden shadow-md border">
              <div className="relative p-3">
                ...
                {p.isGift && (
                  <div className="absolute right-3 top-3 text-[11px] font-bold bg-purple-500 text-white rounded-full px-2 py-0.5">QUÀ TẶNG</div>
                )}
              ...
              <div className="px-4 pb-4">
                ...
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-xs bg-blue-600 text-white px-3 py-1 rounded-full">Thiết bị y tế</span>
                </div>
