    ...
    try {
      ...
      if (!phone || !otp || !password || !email) {
        return res.status(400).json({
          success: false,
          message: 'Phone, OTP, password, and email are required',
        });
      }
  ...
  static async sendOTP(req: Request, res: Response) {
    try {
      ...
      if (method === 'firebase') {
        ...
        const result = await FirebasePhoneService.sendOTP(phone);
        
        if (!result.success) {
          return res.status(500).json({
            success: false,
            message: result.error || 'Failed to send OTP via Firebase',
          });
        }
        ...
        console.log(`🔐 OTP for ${phone}: ${otp} (Method: ${method})`);
